...
class EncoderTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $this->assertEquals(
            "<", $this->encoderInstance->canonicalize("%%33%63", false)
    ...
    {
        $this->assertEquals(
            "<", $this->encoderInstance->canonicalize("%%33c", false)
    ...
    {
        $this->assertEquals(
            "<", $this->encoderInstance->canonicalize("%3%63", false)
    ...
    {
        $this->assertEquals(
            "<", $this->encoderInstance->canonicalize("&&#108;t;", false)
    ...
    {
        $this->assertEquals(
            "<", $this->encoderInstance->canonicalize("%2&#x35;3c", false)
    ...
    public function testDoubleEncodingCanonicalization_14()
    {
        $this->assertEquals(
            "% & <script> <script>",
            $this->encoderInstance->canonicalize("%25 %2526 %26#X3c;script&#x3e; &#37;3Cscript%25252525253e", false)
        );
    }
    ...
    public function testDoubleEncodingCanonicalization_15()
    {
        $this->assertEquals(
            "< < < < < < <", $this->encoderInstance->canonicalize(
                "%26lt; %26lt; &#X25;3c &#x25;3c %2526lt%253B %2526lt%253B %2526lt%253B", false
            )
        );
    ...
    public function testEncodeForHTML_02()
    {
        ...
        $this->assertEquals(
            "a b c d e f&#x9;g", $instance->encodeForHTML(
                "a" . (chr(0)) . "b" . (chr(4)) . "c" . (chr(128)) . "d" . (chr(150)) . "e" . (chr(159)) . "f" . (chr(
                    9
                )) . "g"
            )
        );
    ...
    public function testEncodeForHTML_03()
    {
        $instance = ESAPI::getEncoder();
        $this->assertEquals(
            "a b c d e f&#x9;g h i j&nbsp;k&iexcl;l&cent;m", $instance->encodeForHTML(
                "a" . (chr(0)) . "b" . (chr(4)) . "c" . (chr(128)) . "d" . (chr(150)) . "e" . (chr(159)) . "f" . (chr(
                    9
                )) . "g" . (chr(127)) . "h" . (chr(129)) . "i" . (chr(159)) . "j" . (chr(160)) . "k" . (chr(161)) . "l"
                    . (chr(162)) . "m"
            )
        );
    }
    ...
    public function testEncodeForHTML_11()
    {
        $instance = ESAPI::getEncoder();
        $this->assertEquals(
            "" . (chr(12345)) . (chr(65533)) . (chr(1244)), "" . (chr(12345)) . (chr(65533)) . (chr(1244))
        );
    }
    ...
    public function testEncodeForCSS_03()
    {
        $instance = ESAPI::getEncoder();
        $this->assertEquals(
            "\\21 \\40 \\24 \\25 \\28 \\29 \\3d \\2b \\7b \\7d \\5b \\5d ", $instance->encodeForCSS("!@$%()=+{}[]")
        );
    }
    ...
    public function testEncodeForSQL_MySQL_STD_03()
    {
        ...
        $this->assertEquals(
            "\\b \\n \\r \\t \\Z \\_ \\\" \\' \\\\ \\0 \\%",
            $instance->encodeForSQL($mysqlStdCodec, "\x08 \x0a \x0d \x09 \x1a _ \" ' \\ \x00 \x25")
        );
    }
    ...
    public function testEncodeForXMLAttribute_symbols()
    {
        ...
        $this->assertEquals(
            "&#x20;&#x21;&#x40;&#x24;&#x25;&#x28;&#x29;&#x3d;&#x2b;&#x7b;&#x7d;&#x5b;&#x5d;",
            $instance->encodeForXMLAttribute(" !@$%()=+{}[]")
        );
    }
    ...
    public function testEncodeForBase64_04()
    {
        ...
        $expected = mb_substr($encoded, 0, 76, 'ASCII') . "\r\n" . mb_substr(
            $encoded, 76, mb_strlen($encoded, 'ASCII') - 76, 'ASCII'
        );
    ...
    public function testDecodeFromBase64_01()
    {
        ...
        for ($i = 0; $i < 100; $i++) {
            try {
                ...
                $encoded = ESAPI::getRandomizer()->getRandomString(1, Encoder::CHAR_ALPHANUMERICS)
                    . $instance->encodeForBase64($unencoded, ESAPI::getRandomizer()->getRandomBoolean());
    ...
    public function testWindowsCodec_04()
    {
        ...
        $immune
            = array("\0"); // not that it matters, but the java test would encode alphanums with such an immune param.
    ...
    public function testCharsPlusUnicodeForBase64()
    {
        ...
        for ($i = 0; $i < 256; $i++) {
            ...
            $output = $instance->encodeForBase64($input);
            $this->assertRegExp(
                $expected, $output, "Input was char known as '&thetasym;' concat with character with ordinal: {$i} - %s"
            );
            $this->assertEquals($input, $instance->decodeFromBase64($output));
        }
    }
